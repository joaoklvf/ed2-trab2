...
int main()
{
	...
	while (option != 0)
	{
		...
		else if (option == 0)
		{
			...
			writeOffsetFile(searchKeyRecords);
			fclose(mainFile);
			return 0;
		}
		else
...
int search(loadedFileDto *loadedFiles, FILE *mainFile)
{
	...
	while (tam_reg > 0 && founded == 0)
	{
		...
		if (strcmp(cliente, loadedFiles->keys[indexToRemove].cod_cli) == 0 && strcmp(veiculo, loadedFiles->keys[indexToRemove].cod_vei) == 0)
		{
